    ...
    public class TripConnSummary : IBroker
    {
        ...
        private Broker.BrokerResponse getTripConnSummary(Broker.BrokerRequest paramRequest)
        {
            try
            {
                ...
                if (objConnProcessor.connectionPoints.Count > 0)
                {
                    foreach (TripConnectionPoint objPoint in objConnProcessor.connectionPoints.Values)
                    {
                        ...
                        newRow["COMMENTS"] = ConnectionLabel.getTripComment(wellId, ref paramRequest.objDataService, objPoint.Depth);// DataService.checkNull(objRow["USER_COMMENT"], "").ToString();
                        ...
                        //newRow["DAY_NIGHT"] = objPoint.DayNightStatus;// DataService.checkNull(objRow["TIME"], "D").ToString();
                ...
                //    //DataTable
                //    objExclConn = paramRequest.objDataService.getTable("SELECT DEPTH FROM VMX_TRIP_CONN_INFO WHERE WELL_ID='" + wellId + "'");

                //    // Dictionary<double, double>
                //    exclList = new Dictionary<double, double>();
                ...
                //    //double
                //    TargetCost = objTripConnSettings.TargetTime * CostPerMinute;
            ...
            catch (Exception ex)
            {
                ...
                objLogger.LogMessage("Error in getTripConnections " + ex.Message + ex.StackTrace);
