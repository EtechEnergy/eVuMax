...
import $ from 'jquery';
import { Grid, GridColumn as Column, GridColumn, GridToolbar, GridColumnReorderEvent } from '@progress/kendo-react-grid';
import { Switch, DropDownButtonItem, Tooltip } from '@progress/kendo-react-all';
...
export default class ActiveWell extends React.Component {
    ...
    state = {
        ...
        dataForDropDown: [
            { text: "template-1", value: "openPage" },
            { text: "template-2", value: "openPage" },
            { text: "template-3", value: "openPage" }
        ],
            ...
            axios
                ...
                .then((res) => {
                    ...
                    if (res.data.RequestSuccessfull) {
                        ...
                        this.setState({ columnNames: columnList, activeWellList: _Data.map((dataItem: any) => Object.assign({ selected: false }, dataItem)) });
    ...
    getWellName = () => {
        ...
        let index = wellList.findIndex((x: any) => x.WELL_ID === this.state.currentWellID);
        if (index >= 0) {
    ...
    handleChangeOpenInterface = (event: any) => {
        ...
        if (interfaceID === "ROPSummary") {
            history.push("ROPSummaryPlot/" + this.state.currentWellID);
        }
    ...
    render() {
        ...
        return (
            <>
                    ...
                    >
                        <GridToolbar>
                            <div className="float-left">
                                <label className="text-theme font-weight-bold mr-1" style={headerlabel} > Active Wells  <span className="ml-2">
                            ...
                            <div className="float-right" style={{ width: '35%', display: 'inline-flex' }} >
                                <div>

                                    {this.state.removeWells && <Button type="button" id="btnRemovewell" className="btn-custom btn-custom-primary mr-3" onClick={(e) => this.cmdRemoveWell_click()} >Remove Well</Button>}
                                    <span className="mr-3"><Checkbox onChange={this.tglRemoveWells} checked={this.state.removeWells} label={'Remove wells'} /></span>
                                </div>

                                <div className="k-textbox k-space-right" style={searchFieldStyle}>
                                    <input type="text" onChange={this.filterData} placeholder="Search" />
                        ...
                        {this.state.removeWells && <Column
                            ...
                            headerSelectionValue={
                                this.state.activeWellList.findIndex((dataItem: any) => dataItem.selected === false) === -1
                            }

                        ></Column>}
                        {this.isColumnVisible("DOWNLOAD_STATUS") && (
                            <Column field="DOWNLOAD_STATUS" width="100px" title="Download" resizable={false} minResizableWidth={50} headerClassName="text-center" reorderable={false} orderIndex={this.getColumnOrderIndex("DOWNLOAD_STATUS")}
                                cell={props => (
                                    // onclick = ((event: any) => {
                                    //     // this.setState({
                                    ...
                                    <td className="text-center">
                                        ...
                                        {props.dataItem["DOWNLOAD_STATUS"] == true ?
                                            ...
                                            <span onClick={(e) => this.showDownloadStatusDialog(e, props.dataItem)} ><img src={require("../../../images/downloading-blue.gif")} key="0" style={{ width: 35, height: 10 }} /> </span>
                        ...
                        {this.isColumnVisible("ALARM_STATUS") && (
                            <Column field="ALARM_STATUS" width="100px" title="Alarm Status" resizable={false} minResizableWidth={50} headerClassName="text-center" reorderable={false} orderIndex={this.getColumnOrderIndex("ALARM_STATUS")}
                                cell={props => (
                                    <td className="text-center">
                                        ...
                                        {props.dataItem["ALARM_STATUS"] == 1 ?
                                            <span className="color-yellow"> <FontAwesomeIcon icon={faCircle} /></span>
                                            : null}
                                    </td>)}
                        ...
                        {this.isColumnVisible("WELL_ID") && <Column field="WELL_ID" title="Well ID" width="100px" reorderable={true} />}
                        {this.isColumnVisible("WELL_NAME") && <Column field="WELL_NAME" title="Well Name" width="230px" reorderable={true} orderIndex={this.getColumnOrderIndex("WELL_NAME")}

                            cell={props => (
                                <td className="text-left">
                                    <label onClick={(e) => this.showOpenInterfaceDialog(props.dataItem)} >{props.dataItem.WELL_NAME}</label>
                        ...
                        />}
                        {this.isColumnVisible("BLOCK") && <Column field="BLOCK" headerClassName="text-center" className="text-center" title="Block" width="90px" reorderable={true} orderIndex={this.getColumnOrderIndex("BLOCK")} />}
                        {this.isColumnVisible("REGION") && <Column field="REGION" headerClassName="text-center" className="text-center" title="Region" width="90px" reorderable={true} orderIndex={this.getColumnOrderIndex("REGION")} />}
                        {this.isColumnVisible("DISTRICT") && <Column field="DISTRICT" headerClassName="text-center" className="text-center" title="District" width="90px" reorderable={true} orderIndex={this.getColumnOrderIndex("DISTRICT")} />}
                        {this.isColumnVisible("FIELD") && <Column field="FIELD" title="Field" width="120px" reorderable={true} orderIndex={this.getColumnOrderIndex("FIELD")} />}
                        {this.isColumnVisible("STATE") && <Column field="STATE" headerClassName="text-center" className="text-center" title="State" width="90px" reorderable={true} orderIndex={this.getColumnOrderIndex("STATE")} />}
                        {this.isColumnVisible("COUNTY") && <Column field="COUNTY" headerClassName="text-center" className="text-center" title="County" width="90px" reorderable={true} orderIndex={this.getColumnOrderIndex("COUNTY")} />}
                        {this.isColumnVisible("OPERATOR") && <Column field="OPERATOR" title="Operator" width="90px" reorderable={true} orderIndex={this.getColumnOrderIndex("OPERATOR")} />}

                        {this.isColumnVisible("EDR_PROVIDER") && <Column field="EDR_PROVIDER" title="EDR Provider" width="90px" reorderable={true} orderIndex={this.getColumnOrderIndex("EDR_PROVIDER")} />}
                        {this.isColumnVisible("RIG") && <Column field="RIG_NAME" title="Rig Name" width="200px" reorderable={true} orderIndex={this.getColumnOrderIndex("RIG")}
                            cell={props => (
                                <td className="text-left">
                                    <label onClick={(e) => this.showOpenInterfaceDialog(props.dataItem)} >{props.dataItem.RIG_NAME}</label>

                                </td>
                            )}

                        />}
                        {this.isColumnVisible("RIG_STATE_NAME") && <Column field="RIG_STATE_NAME" title="Current Activity" width="120px" orderIndex={this.getColumnOrderIndex("CURRENT_ACTIVITY")}
                        ...
                        />}
                        {this.isColumnVisible("DEPTH") && <Column field="DEPTH" headerClassName="text-right" className="text-right" title="Depth" width="90px" format="{0:0.00}" orderIndex={this.getColumnOrderIndex("DEPTH")} />}
                        {this.isColumnVisible("HOLE_DEPTH") && <Column field="HDTH" headerClassName="text-right" className="text-right" title="Hole Depth" width="100px" format="{0:0.00}" orderIndex={this.getColumnOrderIndex("HOLE_DEPTH")} />}
                        {/* {this.isColumnVisible("OPEN_INTERFACE") && <Column title="Open Interface" width="150px" orderIndex={this.getColumnOrderIndex("OPEN_INTERFACE")}
                            cell={props => (
                                <td className="text-center">
                                    <DropDownList
                                        style={{ width: "100%" }}

                                        onChange={e => this.OpenInterfaceChange(e, props.dataItem)}
                        ...
                        <Column width="70px" headerClassName="text-center" resizable={false} orderIndex={this.getColumnOrderIndex("EDIT_WELL")}
                            field="editWell"
                            title="Edit Well"
                            cell={props => (
                                <td style={props.style} className={"text-center k-command-cell " + props.className}
                                >
                                    <span onClick={e => this.cmdEditWell_click(e, props.dataItem)}>
                ...
                {
                    this.state.showWellColumnWindow && <Window title={"eVuMax"} initialHeight={500} initialWidth={700} modal={true}
                        minimizeButton={() => null}
                        maximizeButton={() => null}
                        restoreButton={() => null}
                ...
                {/* onClose={this.toggleDialog} */}
                {
                    this.state.showDeleteDialog && <Dialog title={"eVuMax"} onClose={() => this.setState({ showDeleteDialog: false, removeWells: false })} >
                        ...
                        <DialogActionsBar>
                            <button className="btn-custom btn-custom-primary mr-3 mb-3" onClick={() => this.setState({ showDeleteDialog: false, removeWells: false })}>No</button>
                            <button className="btn-custom btn-custom-primary mb-3 mr-3" onClick={this.cmdRemoveWellFromDashboard}>Remove</button>
                        </DialogActionsBar>
                    </Dialog>
                }
                ...
                {this.state.showDownloadStatusDialog &&
                    <Window
                        ...
                        maximizeButton={() => null}
                    >
                        <DownloadStatus WellID={this.state.currentWellID} WellName={this.getWellName()}></DownloadStatus>
                    </Window>
                }
                    ...
                    >
                        ...
                        <div className="row">
                            <div className="col lg-10 sm-6 mr-6 ml-6 mt-4 ">
                                ...
                                <DropDownList
                                    style={{ width: "300px" }}
                                    data={[{ id: "DrlgSummary", name: "Drilling Summary" }, { id: "DrlgConnSummary", name: "Drlg. Conn. Summary" }, { id: "DrlgConnSummary2", name: "Drlg. Conn. Summary (Split View)" }, { id: "TripConnSummary", name: "Trip Conn. Summary" }, { id: "ToolfaceSummary", name: "Toolface Summary" }, { id: "ROPSummary", name: "ROP Summary" }, { id: "Broomstick", name: "Broomstick" }]}
                                    textField="name"
                                    dataItemKey="id"
